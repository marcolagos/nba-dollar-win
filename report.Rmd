...
generate_advanced_stats_model <- function(data, exclude) {
	...
	lasso_model <- glmnet(x = data.matrix(predictor_variables_df), y = data.matrix(wins_col_vector), alpha = 1)
	num_cols <- dim(lasso_model$beta)[2]

	print("==================================================")
	print("Lambda Values tested by LASSO model:")
	print("---------------------------------------------------")
	print(lasso_model$lambda[num_cols])
	print("")
	...
	print("---------------------------------------------------")
	print(lasso_model$dev.ratio[num_cols])
	print("==================================================")
	
	# Creating a plot of the lambda values tested by the Lasso model
	plot(log(lasso_model$lambda), lasso_model$dev.ratio, type = "b", xlab = "Log Lambda", ylab = "Deviance Ratio", main = "Lasso Model: Lambda vs. Deviance Ratio")
	
	# Create a sample matrix
	beta_vals <- lasso_model$beta[,num_cols]
	barplot(beta_vals, col="blue", main="Lasso Model Beta Values", las=2)
	par(cex.axis=0.4, cex.main=0.4)
	
	return (lasso_model)
}

aparam <- c("Rk", "team", "W","L", "PW", "PL", "MOV", "Arena", "wins", "SRS", "ORtg", "DRtg", "NRtg", "TS.", "eFG..1", "eFG.", "FT.FGA")
advanced_stats_lasso_model <- generate_advanced_stats_model(advanced_stats, aparam)
```

```{r}
predict_team_success <- function(team, lasso_model, exclude) {
	...
	num_cols <- dim(lasso_model$beta)[2]
	best_lambda_value <- lasso_model$lambda[num_cols]
	
	print("Number of Regressors:")
	print(dim(advanced_stats_team_wins_predictors)[2])
	
	results <- predict(lasso_model, newx=data.matrix(advanced_stats_team_wins_predictors), s=best_lambda_value, type="response")
	
	plot(advanced_stats_team_wins$year, results, col = "red", xlab = "Year", ylab = cat("Wins for ", team, ""))
...
results <- predict_team_success("houston rockets", advanced_stats_lasso_model, aparam)

```
Applying our model to future data
```{r}
predict_on_future_data <- function(future_data, lasso_model, exclude) {
	...
	results <- predict(lasso_model, newx=data.matrix(future_data_predictors), s=best_lambda_value, type="response")
	
	team_inds <- seq(from=1, to=length(future_data$team), by=1)
	plot(team_inds, results, col = "red", xlab = "Team", ylab = new_year_string, ylim=c(10, 90))
	text(seq_along(future_data$team), rep(0.5, length(future_data$team)), labels = future_data$team, srt = 90, adj = c(0, 0.5), xpd = TRUE, cex = 0.4)
...
result_future <- predict_on_future_data(advanced_stats_2021, advanced_stats_lasso_model, aparam)
